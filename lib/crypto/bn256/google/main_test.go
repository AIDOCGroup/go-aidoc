...
func TestPairings(t *testing.T) {
	...
	if p0.String() != p0_3.String() {
		t.Error("配对不匹配： e(a, b) 有错误的顺序")
	}
	p2 := Pair(a2, b1)
	p2_2 := Pair(a1, b2)
	p2_3 := new(GT).ScalarMult(p1, bigFromBase10("2"))
	if p2.String() != p2_2.String() {
		t.Error("配对不匹配： e(a, b * 2) != e(a * 2, b)")
	}
	if p2.String() != p2_3.String() {
		t.Error("配对不匹配： e(a, b * 2) != e(a, b) ** 2")
	}
	if p2.String() == p1.String() {
		t.Error("配对是退化的！") //英 Pairing is degenerate!
	}
	if PairingCheck([]*G1{a1, a1}, []*G2{b1, b1}) {
		t.Error("MultiAte 检查给出了误报！")
	}
	p999 := Pair(a37, b27)
	p999_2 := Pair(a1, b999)
	if p999.String() != p999_2.String() {
		t.Error("配对不匹配： e(a * 37, b * 27) != e(a, b * 999)")
	}
}
