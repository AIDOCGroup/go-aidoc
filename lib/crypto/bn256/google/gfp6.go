...
//有关所用算法的详细信息，请参阅“配对友好字段的乘法和平方”，Devegili等。
...
// gfP6 将大小为 p⁶ 的场实现为 gfP2 的立方扩展，其中 τ³=ξ 且 ξ= i + 9。
type gfP6 struct {
	x, y, z *gfP2 // 值是 xτ²+yτ+ z
}
...
// FrobeniusP2 计算（xτ²+yτ+ z）^(p²）= xτ^(2p²) + yτ^(p²) + z
...
func (e *gfP6) Mul(a, b *gfP6, pool *bnPool) *gfP6 {
	// “在配对友好字段上的乘法和平方”第4节，Karatsuba方法。
	// http://eprint.iacr.org/2006/471.pdf
...
func (e *gfP6) Invert(a *gfP6, pool *bnPool) *gfP6 {
	// 参见“实施加密配对”，M。Scott，第3.2节。
	// ftp://136.206.11.249/pub/crypto/pairings.pdf

	// 这里我们可以简单解释它是如何工作的：让j为立方根
	// GF(p²) 中的单位，使 1 + j +j²= 0。
	// 然后（xτ²+yτ+ z)(xj²τ²+yjτ+ z)(xjτ²+yj²τ+ z)
	// =(xτ²+yτ+ z)(Cτ²+Bτ+ A)
	// =(x³ξ²+y³ξ+z³-3ξxyz)= F是基本字段的元素（范数）。
	//
	// 另一方面(xj²τ²+yjτ+ z)(xjτ²+yj²τ+ z)
	// = τ² (y² - ξxz) + τ(ξx² - yz) + (z² - ξxy)
	//
	// 这就是A =（z² - ξxy），B =（ξx² - yz），C =（y² - ξxz）的原因
	t1 := newGFp2(pool)
